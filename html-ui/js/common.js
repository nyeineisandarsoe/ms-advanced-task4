...
var tasks_url = 'http://localhost/api/task'

var cart_url = 'http://localhost/api/carts' //the cart api url was unknown in this moment, I assume this is the url
...
function checkout(e) {
	...
	axios.post(cart_url, data).then(res => {
		const data = res.data;
...
function getTasks() {
	axios.get(tasks_url).then(response => {
		...
		response.data.forEach(function (task) {
			...
			var current = i++;
			const td1 = document.createElement('td'); //Task Id
			td1.innerHTML = response.data[current].Id;
			const td2 = document.createElement('td'); //Description
			td2.innerHTML = response.data[current].description;
			const td3 = document.createElement('td'); //Priority
			td3.innerHTML = response.data[current].priority;
			const td4 = document.createElement('td'); //Status
			td4.innerHTML = response.data[current].Status;
